...
const InitialContractCard = ({ name, businessName, description, profile, completed, businessData, url }) => {
    ...
    return (
        <>
            ...
            <div className='border p-3 rounded-4 shadow1 d-flex flex-column h-100 justify-content-between'>
                <div onClick={() => handleViewAdd("/view-contract", businessData)} className='' style={{ cursor: "pointer" }} >
                ...
                <div>
                    {completed === false && (<div className='mt-4 display_flex2 gap-3 flex-wrap'>
                        <button onClick={() => handleViewAdd("/view-contract", businessData)} className=' btn1 btn_minWidth fs_07 btn_primary rounded-3 px-3 py-2'>
                            {t("VIEW_CONTRACT")}
                        </button>
                        <button onClick={() => handleViewAdd("/business-detail", businessData?.business)} className=' btn1 btn_minWidth fs_07 btn_primary_outline rounded-3 px-3 py-2'>
                            {t("VIEW_BUSINESS")}
                        </button>
                    </div>)}
                    {completed === true &&
                        (<>
                            {businessData?.is_business_exist ?
                                <div className='mt-2'>
                                    {userData.user_type === "investor" && (businessData?.is_business_exist?.investor_signature ?
                                        <>
                                    ...
                                    {userData.user_type === "business" && (businessData?.is_business_exist?.business_signature ?
                                        <>
                                            <div className='text-danger popins_semibold text-center fs_09'>
                                                {t("ALREADY_CREATED")}
                                            </div>
