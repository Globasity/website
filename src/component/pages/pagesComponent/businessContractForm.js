...
const BusinessContractForm = ({
  ...
  const handleBusinessCreate = (businessData2, contractDetail) => {
    navigate("/business-contract-view", {
      state: {
    ...
    if (name === "share_issued_percent" || name === "investment_percent") {
      if (value > 100) {
        setErrorMessage((prevErrors) => ({
          ...prevErrors,
          [name]: `Value must be 100 or less.`,
        }));
      } else {
  ...
  return (
    <div>
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          <Form
            className="px-2 gap-1 d-flex flex-column"
            onSubmit={checkUpdate === true ? updateData : handleSubmit}
          >
            <Form.Group controlId="days">
              <Form.Label className=" fs_08 popins_medium mt-2 mb-1">
                Days
              </Form.Label>
              <Form.Control
                required
            ...
            <Form.Group controlId="investmentShare">
              ...
              <Form.Control
                ...
                value={formData.investment_share}
                onChange={handleChange}
              />
            </Form.Group>
            <Form.Group controlId="investmentPercent">
              ...
              <Form.Control
                ...
                value={formData.investment_percent}
                onChange={handleChange}
              />
              {errorMessage?.investment_percent && (
                <h6 className="fs_07 text-danger">
                  {errorMessage?.investment_percent}
                </h6>
              )}
            </Form.Group>
            ...
            <Form.Group controlId="shareEachValue">
              ...
              <Form.Control
                ...
                className="fs_09"
                type="number"
                name="share_each_value"
                value={formData.share_each_value}
                onChange={handleChange}
              />
            </Form.Group>
            <Form.Group controlId="shareIssuedPercent">
              <Form.Label className=" fs_08 popins_medium mt-2 mb-1">
                Share Issued Percent
              </Form.Label>
              <Form.Control
                required
                className="fs_09"
                type="number"
                name="share_issued_percent"
                value={formData.share_issued_percent}
                onChange={handleChange}
              />
              {errorMessage?.share_issued_percent && (
                <h6 className="fs_07 text-danger">
                  {errorMessage?.share_issued_percent}
                </h6>
              )}
            ...
            <Form.Group>
              <div className="d-flex my-3">
                <button
                  ...
                  type="submit"
                  className="btn1 mx-auto btn2 btn2 fs_09 btn_primary rounded_3 px-4 py-2"
                >
                  {isLoading ? (
